# .pizza
resource "aws_route53_zone" "dot_pizza" {
  name = var.domain_name
  tags = var.common_tags
}

resource "aws_route53_record" "dot_pizza-a" {
  zone_id = aws_route53_zone.dot_pizza.zone_id
  name    = var.domain_name
  type    = "A"

  alias {
    name                   = aws_cloudfront_distribution.dot_pizza-www.domain_name
    zone_id                = aws_cloudfront_distribution.dot_pizza-www.hosted_zone_id
    evaluate_target_health = false
  }
}

resource "aws_route53_record" "dot_pizza-www-a" {
  zone_id = aws_route53_zone.dot_pizza.zone_id
...
resource "aws_route53_record" "dot_pizza-cert_validation" {
  for_each = {
    for dvo in aws_acm_certificate.dot_pizza.domain_validation_options : dvo.domain_name => {
